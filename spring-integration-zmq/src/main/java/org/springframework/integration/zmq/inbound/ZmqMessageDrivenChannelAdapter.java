...
package org.springframework.integration.zmq.inbound;
...
import org.springframework.integration.zmq.event.ZmqConnectionFailedEvent;
 ...
 *
 */
public class ZmqMessageDrivenChannelAdapter extends AbstractZmqMessageDrivenChannelAdapter
	 ...
	 * if the server URI is provided by the {@link DefaultZmqClientFactory}.
	 * @param url the URL.
	 * @param clientId The client id.
	 * @param clientFactory The client factory.
	 */
	public ZmqMessageDrivenChannelAdapter(String url, String clientId,
										ZmqClientFactory clientFactory) {
		super(url, clientId);
	 ...
	 * @since 5.0.1
	 */
	public ZmqMessageDrivenChannelAdapter(String clientId, ZmqClientFactory clientFactory) {
		super(null, clientId);
		this.clientFactory = clientFactory;
	}
	...
	public synchronized void connectionLost(Throwable cause) {
		...
		if (this.applicationEventPublisher != null) {
			this.applicationEventPublisher.publishEvent(new ZmqConnectionFailedEvent(this, cause));
		}
	}
	...
	private synchronized void connectAndSubscribe() {
		...
		try {
			...
			if (this.applicationEventPublisher != null) {
				this.applicationEventPublisher.publishEvent(new ZmqConnectionFailedEvent(this, e));
			}
		...
		if (this.applicationEventPublisher != null) {
			this.applicationEventPublisher.publishEvent(new ZmqSubscribedEvent(this, message));
		}
		...
		try {
			this.reconnectFuture = getTaskScheduler().schedule(() -> {
				try {
					...
					synchronized (ZmqMessageDrivenChannelAdapter.this) {
						if (!ZmqMessageDrivenChannelAdapter.this.connected) {
							connectAndSubscribe();
							ZmqMessageDrivenChannelAdapter.this.reconnectFuture = null;
						}
					}
				}
				catch (ZMQException e) {
